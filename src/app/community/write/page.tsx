...
export default function CommunityWritePage() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (user) {
        ...
        router.push("/profile"); // 로그인 안 되어 있으면 프로필(로그인)로 유도
      }
    };
    checkUser();
  }, [supabase, router]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1. 이미지 업로드 (기존 상품 버킷인 item-images 재사용)
    ...
    <>
      <div className="flex flex-col h-[100dvh] bg-white">
        ...
        <header className="flex justify-between items-center px-4 h-14 border-b border-gray-100 sticky top-0 bg-white/90 backdrop-blur-md z-10">
          ...
          <h1 className="text-[17px] font-bold text-gray-900 tracking-tight">동네생활 글쓰기</h1>
          <button
