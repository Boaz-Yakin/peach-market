...
1. [x] **안심 결제 Flow 설계:** `transactions` 테이블 고도화 및 수수료 계산 로직 완료.
2. [ ] **지역 광고주 CMS:** 로컬 비즈니스가 직접 이미지를 올릴 수 있는 심플한 대시보드.

### Phase C: 신뢰 자산화 및 로컬 커뮤니티 강화 (Planning)
1. [/] **매너 온도 (Peach Brix) 엔진 구축:** 리뷰 제출 시 당도 실시간 반영 로직 개발 중.
2. [ ] **거래 완료 후 리뷰 유도:** 채팅창 내 자동 리뷰 작성 가이드 제공 예정.
