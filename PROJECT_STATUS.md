...
1. **사용자 인증**: Google Social Login (Supabase Auth)
2. **거래 게시판**: 물건 등록(이미지 업로드 포함), 수정, 삭제, 상세 보기
3. **지능형 검색**: 제목, 내용, 위치 기반 통합 검색 및 카테고리 필터링
4. **실시간 채팅**: 구매자와 판매자 간 1:1 채팅 (Realtime)
5. **찜하기 시스템**: 관심 상품 저장 및 위시리스트 관리
6. **거래 상태 관리**: 판매중 / 예약중 / 판매완료 상태 전환 및 뱃지 표시
7. **유저 대시보드**: 나의 활동 현황(판매글, 찜, 채팅) 요약

---

## 🚀 향후 로드맵 (Future Roadmap)
- [ ] **매너 온도**: 유저 간 거래 후기 및 평점 시스템
